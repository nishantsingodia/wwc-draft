...
- **Identity-first player matching.** Points and XI membership join on a stable registry `pid` — the **cricinfo id** (`ci:<cricinfoId>`; migrated 25 Jul 2026), with a shared fuzzy name-matcher (`lib/fuzzy-name-match.ts`) as a fallback that returns `null` on ambiguity rather than guessing — this is what fixed same-surname mis-joins (e.g. Sunny vs Smit Patel, and the Ash/Ashleigh Gardner split).
